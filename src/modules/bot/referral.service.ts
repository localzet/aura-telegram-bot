...
export class ReferralService {
  ...
  async onRef(@Ctx() ctx: Context): Promise<void> {
    try {
      ...
      const { referredCountThisMonth, firstDiscount } = await getPrice(
        1,
        user,
        this.prisma,
        this.config,
  ...
  async onRefManage(@Ctx() ctx: Context): Promise<void> {
    try {
      ...
      if (!["aurum", "platinum"].includes(user.level)) {
        ...
        log(
          `onRefManage: access denied for user ${user.telegramId.toString()}`,
        );
        return;
      }
...
`,
      ...
      log(
        `onRefManage: management panel shown for user ${user.telegramId.toString()}`,
      );
    } catch (error) {
  ...
  async onPromote(@Ctx() ctx: Context): Promise<void> {
    try {
      ...
      const targetName =
        target.fullName || target.username || target.telegramId.toString();
