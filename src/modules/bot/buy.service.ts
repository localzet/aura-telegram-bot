...
🎁 Ваша скидка: ${user.discount}%
    ...
    async successfulPayment(@Ctx() ctx: Context): Promise<void> {
        try {
            ...
            if (!months) {
                this.logger.warn(`Не найден срок подписки для платежа ${payment?.invoice_payload}`);
                return;
            }
            ...
            if (success) {
                await ctx.reply(`✅ Подписка активна до ${success.toLocaleDateString('ru-RU')}`);
            } else {
    ...
    private async extendSubscription(ctx: Context, expireAt: Date | null, months: number): Promise<Date | null> {
    ...
    private async createSubscription(ctx: Context, months: number): Promise<Date | null> {
        const today = this.today();
