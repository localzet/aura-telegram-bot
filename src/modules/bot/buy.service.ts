...
<pre>${err.message}</pre>`);
    ...
    async checkout(@Ctx() ctx: Context): Promise<void> {
        try {
            ...
            if (!months) {
                this.logger.warn(
                    `Не найден срок подписки для платежа ${payload}`,
                );
                return;
            }
...
<b>Months:</b> ${months}`;
                ...
                await ctx.answerPreCheckoutQuery(false, {
                    error_message:
                        "Не удалось подготовить аккаунт. Попробуйте позже.",
                });
                return;
            }
...
<pre>${err.message}</pre>`);
    ...
    async successfulPayment(@Ctx() ctx: Context): Promise<void> {
        try {
            ...
            if (expireDate) {
                await this.notifyDev(`⚠️ Оплата прошла, но дата окончания подписки не найдена
<b>User:</b> ${ctx.from?.id}
...
<b>User:</b> ${ctx.from?.id}
<pre>${err.message}</pre>`);
            await ctx.reply("⚠️ Оплата прошла, но при активации произошла ошибка. Мы решим вопрос в ближайшее время.");
        }
    }
    ...
    private async notifyDev(message: string): Promise<void> {
        try {
            ...
            if (!adminId) {
                this.logger.warn("ADMIN_TG_ID не задан в конфиге, уведомление не отправлено");
                return;
            }
            await this.bot.api.sendMessage(adminId, message, {parse_mode: "HTML"});
        } catch (e: any) {
            this.logger.error(`Не удалось отправить уведомление разработчику: ${e.message}`, e.stack);
        }
    }
}
