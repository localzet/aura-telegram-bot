...
export class WebhookService {
  ...
  async handleEvent(event: string, data: any) {
    ...
    if (type === "user") {
      ...
      if (!tgId) {
        this.logger.warn(
          `⚠️ Пользователь ${info.username} не привязан к Telegram`,
        );
        return;
      }
      ...
      const messages: Record<string, string> = {
        ...
        disabled:
          "🔒 [SYSTEM] Ваша учётная запись *отключена*. Обратитесь в поддержку.",
        enabled:
          "✅ [SYSTEM] Ваша учётная запись *активирована*. Добро пожаловать!",
        // limited: '📉 [SYSTEM] Ваша учётная запись *ограничена*. Некоторые функции могут быть недоступны.',
        expired:
          "⌛ [SYSTEM] Ваша *подписка закончилась*. Пожалуйста, продлите её.",
        // traffic_reset: '🔄 [SYSTEM] Ваш трафик был сброшен.',
        expires_in_72_hours:
          "⏳ [SYSTEM] Подписка заканчивается *через 3 дня*. Пожалуйста, продлите её.",
        expires_in_48_hours:
          "⏳ [SYSTEM] Подписка заканчивается *через 1 день*. Пожалуйста, продлите её.",
        expires_in_24_hours:
          "⚠️ [SYSTEM] Подписка заканчивается *через 24 часа*. Пожалуйста, продлите её.",
  ...
  async notifyEvent(message: string): Promise<void> {
    try {
      ...
      await this.botService.sendMessage(adminId, `<code>${message}</code>`, {
        parse_mode: "HTML",
      });
    } catch (e: any) {
      this.logger.error(
