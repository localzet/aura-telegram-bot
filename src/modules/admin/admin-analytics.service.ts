...
export class AdminAnalyticsService {
  ...
  async getFinancialAnalytics(startDate?: Date, endDate?: Date) {
    ...
    const byLevel = purchases.reduce(
      (acc, p) => {
    ...
    const byMonth = purchases.reduce(
      (acc, p) => {
  ...
  async getTariffRating() {
    ...
    const levelRevenue = purchases.reduce(
      (acc, p) => {
        const level = p.user?.level || "ferrum";
        if (!acc[level]) {
      ...
      {} as Record<
        string,
        { revenue: number; purchases: number; users: number }
      >,
    );
    ...
    const monthStats = purchases.reduce(
      (acc, p) => {
        const month = p.month;
