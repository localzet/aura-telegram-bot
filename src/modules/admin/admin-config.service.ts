...
export class AdminConfigService {
  ...
  async getConfig(): Promise<Record<string, any>> {
    ...
    return {
      ...
      closedMode:
        dbConfig.find((c) => c.key === "CLOSED_MODE_ENABLED")?.value ===
          "true" || this.config.get<boolean>("CLOSED_MODE_ENABLED", false),
  ...
  async updatePricingConfig(
    ...
    if (config.discounts) {
      if (config.discounts[3] !== undefined) {
        await this.updateConfig(
          "PRICE_DISCOUNT_3_MONTHS",
          String(config.discounts[3]),
      ...
      if (config.discounts[6] !== undefined) {
        await this.updateConfig(
          "PRICE_DISCOUNT_6_MONTHS",
          String(config.discounts[6]),
      ...
      if (config.discounts[12] !== undefined) {
        await this.updateConfig(
          "PRICE_DISCOUNT_12_MONTHS",
          String(config.discounts[12]),
    ...
    if (config.referral) {
      if (config.referral.bonusPercent !== undefined) {
        await this.updateConfig(
          ...
          String(config.referral.bonusPercent),
          "Referral bonus percent per user",
          updatedBy,
        );
      }
      if (config.referral.maxBonus !== undefined) {
        await this.updateConfig(
          "REFERRAL_MAX_BONUS",
          String(config.referral.maxBonus),
          "Max referral bonus percent",
          updatedBy,
        );
      }
    }
  ...
  async updateClosedMode(enabled: boolean, updatedBy?: string): Promise<void> {
    await this.updateConfig(
      "CLOSED_MODE_ENABLED",
      enabled ? "true" : "false",
      "Enable/disable closed mode (invitation only)",
      updatedBy,
    );
  }
