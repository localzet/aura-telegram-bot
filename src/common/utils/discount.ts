...
async function getPricingFromDB(prisma: PrismaService, config: ConfigService): Promise<PricingConfig> {
    ...
    const getValue = (key: string, defaultValue: any) => {
        return configMap[key] !== undefined ? parseFloat(configMap[key]) : config.get(key, defaultValue);
    };
    ...
    return {
        ...
        discounts: {
            ...
            6: getValue('PRICE_DISCOUNT_6_MONTHS', 0.80),
...
export async function getPrice(
    ...
    if (pricingConfig) {
        ...
        pricing = {
            basePrice: 180,
            discounts: { 1: 1, 3: 0.85, 6: 0.80, 12: 0.75 },
            levels: {
    ...
    const base = pricing.basePrice * pricing.discounts[months as keyof typeof pricing.discounts];
