...
import { IconSettings, IconDeviceFloppy, IconAlertCircle } from '@tabler/icons-react';
    ...
    const loadConfig = async () => {
        ...
        try {
            ...
            notifications.show({
                color: 'red',
                title: 'Ошибка',
                message: error.response?.data?.message || 'Не удалось загрузить конфигурацию',
            });
        } finally {
            setLoading(false);
        }
    ...
    const handleSavePricing = async () => {
        ...
        try {
            ...
            notifications.show({
                color: 'red',
                title: 'Ошибка',
                message: error.response?.data?.message || 'Не удалось сохранить конфигурацию',
            });
        } finally {
            setSaving(false);
        }
                ...
                {pricing && (
                    <Paper p="md" withBorder>
                        <Stack gap="md">
                            ...
                            {(['ferrum', 'argentum', 'aurum', 'platinum'] as const).map((level) => (
                            ...
                            <Alert icon={<IconAlertCircle size={16} />} color="yellow" mt="md">
                                Изменения вступят в силу после сохранения. Текущие цены будут пересчитаны при следующей покупке.
                            </Alert>
