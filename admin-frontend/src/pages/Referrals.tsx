    ...
    try {
      ...
      notifications.show({
        ...
        message:
          ...
          "Не удалось загрузить реферальную сеть",
      });
    } finally {
      setLoading(false);
    }
      ...
      {network && network.nodes.length > 0 ? (
        <>
          <Paper p="md" withBorder>
            <Text fw={700} mb="md">
              Граф реферальной сети ({network.nodes.length} узлов,{" "}
              {network.edges.length} связей)
            </Text>
            <div
              style={{
              ...
              {viewMode === "2d" ? (
                <ForceGraph2D
                  ...
                  nodeCanvasObject={(
                    ...
                    const label =
                      node.username || node.telegramId?.toString() || node.id;
          ...
          <Stack gap="xs">
            {stats.topInviters.map((item: any, index: number) => (
              <Group
                ...
                p="xs"
                style={{ border: "1px solid #373a40", borderRadius: "4px" }}
              >
                <div>
                  <Text fw={500}>
                    {item.user?.fullName ||
