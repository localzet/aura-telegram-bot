...
import (
	...
	"aura-telegram-bot/internal/handler"
	"aura-telegram-bot/internal/notification"
...
func main() {
	...
	auraClient := aura.NewClient(config.AuraUrl(), config.AuraToken(), config.AuraMode())
	...
	paymentService := payment.NewPaymentService(tm, purchaseRepository, auraClient, customerRepository, b, cryptoPayClient, yookasaClient, referralRepository, cache)
	...
	syncService := sync.NewSyncService(auraClient, customerRepository)
