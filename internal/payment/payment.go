...
import (
	"aura-telegram-bot/internal/aura"
...
func (s PaymentService) ProcessPurchaseById(ctx context.Context, purchaseId int64) error {
	...
	user, err := s.auraClient.CreateOrUpdateUser(ctx, customer.ID, customer.TelegramID, config.TrafficLimit(), purchase.Month*30)
	if err != nil {
		return err
	}
	...
	refereeUser, err := s.auraClient.CreateOrUpdateUser(ctxReferee, refereeCustomer.ID, refereeCustomer.TelegramID, config.TrafficLimit(), config.GetReferralDays())
	if err != nil {
		return err
	}
...
func (s PaymentService) ActivateTrial(ctx context.Context, telegramId int64) (string, error) {
	...
	user, err := s.auraClient.CreateOrUpdateUser(ctx, customer.ID, telegramId, config.TrialTrafficLimit(), config.TrialDays())
	if err != nil {
